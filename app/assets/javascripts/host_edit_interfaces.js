...
function active_interface_forms() {
  return $.grep($('#interfaceForms').children(), function(f) {
...
function confirm_flag_change(element, element_selector, massage) {
  ...
  var other_primary;
  other_primary = $('#interfaceForms ' + element_selector + ':checked').closest('fieldset');
  other_primary = $.grep(other_primary, function(i) {
    return ($(i).parent().data('interface-id') != this_interface_id);
  });

  if (other_primary.length > 0) {
